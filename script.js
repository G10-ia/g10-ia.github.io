document.addEventListener("DOMContentLoaded", () => {
  ...
  const selectOrdenActividadAdmin = document.getElementById("selectOrdenActividadAdmin");
  ...
  async function registrarActividadSistema(perfil, userId) {
    ...
    try {
      ...
      const { data: existente, error: errorConsulta } = await supabase
        ...
        .select("id, usuario, fecha_ingreso, hora_ingreso, contador_ingresos")
        .eq("user_id", userId)
        .maybeSingle();
      ...
      if (!existente) {
        ...
        const { error: errorHistorialInsert } = await supabase
          .from("actividad_sistema_historial")
          .insert([
            {
              user_id: userId,
              usuario: perfil.usuario || "",
              fecha_hora_ingreso: ultimaConexion
            }
          ]);

        if (errorHistorialInsert) {
          console.error("Error insertando actividad_sistema_historial:", errorHistorialInsert);
        }

        return;
      }

      const contadorActual = Number(existente.contador_ingresos || 0);
      const payloadActividad = {
        usuario: perfil.usuario || existente.usuario || "",
        ultima_conexion: ultimaConexion,
        contador_ingresos: contadorActual + 1
      };
      ...
      if (errorHistorial) {
        console.error("Error insertando historial de actividad:", errorHistorial);
      }
    } catch (error) {
  ...
  function obtenerTimestampActividad(valor) {
    if (!valor) return 0;
    const timestamp = new Date(valor).getTime();
    return Number.isNaN(timestamp) ? 0 : timestamp;
  }

  function obtenerActividadOrdenada(registros) {
    const items = Array.isArray(registros) ? [...registros] : [];

    items.sort((a, b) => {
      const tsA = obtenerTimestampActividad(a?.ultima_conexion);
      const tsB = obtenerTimestampActividad(b?.ultima_conexion);

      if (ordenActividadAdmin === "asc") {
        return tsA - tsB;
      }
  ...
  function obtenerHistorialActividadOrdenado(registros) {
    const items = Array.isArray(registros) ? [...registros] : [];

    items.sort((a, b) => {
      const tsA = obtenerTimestampActividad(a?.fecha_hora_ingreso);
      const tsB = obtenerTimestampActividad(b?.fecha_hora_ingreso);
  ...
  function construirHtmlDetalleActividadAdmin(userId) {
    const registros = obtenerHistorialActividadOrdenado(historialActividadCache[userId] || []);

    if (!registros.length) {
      return "<div class='actividad-detalle-vacio'>No hay ingresos detallados registrados todavÃ­a</div>";
    }

    return `
      <div class="actividad-detalle-encabezado" style="display:flex; justify-content:flex-end; margin-bottom:10px;">
        <button type="button" class="btn-actividad-orden-detalle" data-user-id="${escaparHtml(userId)}" style="height:34px; padding:0 10px; border:1px solid #d7deea; border-radius:999px; background:#f7f9fc; color:#355a8a; font-size:13px; font-weight:700; box-shadow:none;">
          ${ordenActividadAdmin === "asc" ? "MÃ¡s antiguo" : "MÃ¡s reciente"}
        </button>
      </div>
      <div class="actividad-detalle-lista">
        ${registros.map((item) => `
          <div class="actividad-detalle-item">
            <div class="actividad-detalle-fecha">${escaparHtml(formatearFechaActividad(item.fecha_hora_ingreso))}</div>
            <div class="actividad-detalle-hora">${escaparHtml(formatearHoraActividadDesdeTimestamp(item.fecha_hora_ingreso))}</div>
          </div>
        `).join("")}
      </div>
    `;
  ...
  function construirHtmlActividadAdmin(registros) {
    return (registros || []).map((registro) => {
      const estaExpandido = detalleActividadExpandidoUserId === registro.user_id;
      const detalleHtml = estaExpandido
        ? construirHtmlDetalleActividadAdmin(registro.user_id)
        : "";

      return `
        <div class="actividad-admin-tarjeta">
          <div class="actividad-admin-tarjeta-texto">
            ...
            <div><strong>Ingresos:</strong> ${escaparHtml(String(registro.contador_ingresos || 0))}</div>
          </div>
          <div class="actividad-admin-tarjeta-acciones">
            <button type="button" class="btn-actividad-detalle" data-user-id="${escaparHtml(registro.user_id || "")}">
              ${estaExpandido ? "Ocultar detalle" : "Ver detalle"}
            </button>
          </div>
          ${estaExpandido ? `<div class="actividad-admin-detalle">${detalleHtml}</div>` : ""}
        </div>
      `;
    }).join("");
  }
  ...
  function renderizarActividadAdmin() {
    ...
    const registrosOrdenados = obtenerActividadOrdenada(actividadAdminCache);
    listaActividadAdmin.innerHTML = construirHtmlActividadAdmin(registrosOrdenados);
    actualizarResumenActividad(registrosOrdenados.length);
  }

  async function cargarDetalleActividadUsuario(userId) {
    if (!userId || historialActividadCache[userId]) {
      return;
    }
  ...
  async function toggleDetalleActividadAdmin(userId) {
    ...
    await cargarDetalleActividadUsuario(userId);
    renderizarActividadAdmin();
  }
  ...
  async function cargarActividadAdmin() {
    ...
    try {
      ...
      const { data: actividad, error: errorActividad } = await supabase
        .from("actividad_sistema")
        .select("id, user_id, usuario, fecha_ingreso, hora_ingreso, ultima_conexion, contador_ingresos")
